...
const AZURE_API_KEY = process.env["AZURE_API_KEY"]; // Retrieve Azure OpenAI API key from environment variables

if (!AZURE_API_KEY) {
  throw new Error(
    "The AZURE_API_KEY environment variable is missing or empty."
  );
}

// Initialize the OpenAI client with Azure-specific configurations
const openai = new OpenAI({
  apiKey: AZURE_API_KEY, // Use the Azure OpenAI API key
  ...
  defaultHeaders: { "api-key": AZURE_API_KEY }, // Set the API key in headers
});
